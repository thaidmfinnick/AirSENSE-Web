...
class ExamDetail extends Component {
  constructor(props) {
      super(props);
      this.state = {
        data:null,enableDialogue:false,content:""
      };
  }
    ...
    if(!!this.state.data){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          updateCourseToFist(this.state.data)
          .then((value)=>{
    ...
    return (
      <div>
         <TableDataView
          ...
          <br/>
          <button onClick={()=>{this.selectEditPages(true)} }> Edit Page </button>
          <button onClick={()=>{this.selectPagesToTop()} }> Đưa bài lên top </button>
      </div>
