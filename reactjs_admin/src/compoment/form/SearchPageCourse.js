...
const SearchPageCourse =({ id_select,sub_id_select ,changeID})=> {
        var pagesCourse =ManagerData.getTable("pages_course");
        if(sub_id_select!=0) pagesCourse= pagesCourse.filter(o=>o.course_id==sub_id_select);
        ...
        return (
            <FormControl variant="outlined" className={'input-pages-register'}>
                ...
                >
                    ...
                    {pagesCourse ? pagesCourse.map((vars) => (
                        <MenuItem value={vars.pages_course_id} key={vars.pages_course_id}>
                        {vars.title}
                        </MenuItem>
                    )):""}
                </Select>
