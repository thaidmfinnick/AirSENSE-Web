...
const SearchPageData =({ id_select,sub_id_select ,changeID})=> {
        var pagesContent =ManagerData.getTable("content_page");
        if(sub_id_select!=0) pagesContent= pagesContent.filter(o=>o.content_sub_id==sub_id_select);
            ...
            <FormControl variant="outlined" className={'input-pages-register'}>
                ...
                >
                    ...
                    {pagesContent ? pagesContent.map((vars) => (
                        <MenuItem value={vars.content_page_id} key={vars.content_page_id}>
                        {vars.title}
                        </MenuItem>
                    )):""}
                </Select>
