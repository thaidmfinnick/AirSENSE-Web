...
const SelectGroupContentSub = ({ detailValue, onChange }) => {
  ...
  var idSelect = 0;
  var item = content_sub.filter(
    (o) => o.content_sub_id == detailValue
  );
  if (item.length > 0) {
  ...
  content_sub = content_sub.filter(
    (o) => o.content_group_id == idSelect
  );
      ...
      <FormControl variant="outlined" className={'input-pages-register'}>
        ...
        >
          {state.content_sub.map((vars) => (
            <MenuItem
              value={vars.content_sub_id}
              key={vars.content_sub_id}
            >
              {vars.content_group}
            </MenuItem>
          ))}
