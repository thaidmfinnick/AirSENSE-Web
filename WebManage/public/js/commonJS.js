...
function setFormToShowCourse(item, urlDetail) {
    var start =
        '<div class="course-new__item-wrapper col l-3 m-4 c-6" style="margin:0!important; padding-top:4px;">';
    var limkUrl = '<a href="' + urlDetail;
    if (item.is_main_pages_id == -1) {
        limkUrl +=
            'group_lesson/' +
...
function getInfoDetailCourse(nameDivControl, dataView, urlDetail) {
    $.ajax({
        ...
        success: (data) => {
            ...
            console.log(dataJson);
            var titleSub = '';
            var startRow = '<div class="row js-course-section-scroll">';
            var controlPage =
                '<div class="course__control-page"><div class="course__prev-page course__control-page--disable"><i class="fal fa-chevron-left"></i><span class="">PREV</span></div><div><span class="course__page-number course__control-page--active">1</span><span class="course__page-number">2</span></div><div class="course__next-page"><span>NEXT</span> <i class="fal fa-chevron-right"></i></div></div>';
            var closeRow = '</div>' + controlPage + '</div>';
            ...
            for (var i = 0; i < dataJson.length; i++) {
                if (dataJson[i].course_group != titleSub) {
                    if (dataJson[i] == 0) {
                        ...
                        textHtml +=
                            '<div class="course-section"><div class="course__heading-wrapper"><h2 class="course__heading">' +
                            titleSub +
