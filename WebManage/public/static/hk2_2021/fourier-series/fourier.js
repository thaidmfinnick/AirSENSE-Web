...
function createRow() {
  ...
  var num = document.getElementById('num').value;
  num = num - 0;
  var result = document.querySelector('.container-i');
  //console.log(test);
  for (var i = 0; i <= num; i++) {
    //order
    result.innerHTML = result.innerHTML + addChild(i, valueA[i], valueB[i]);
  }
}

function addChild(order, an, bn) {
  return `<div class="grid-i box-i">
  <p class="order fourier">${order}</p>
  <p class="an fourier">${an}</p>
  <p class="bn fourier">${bn}</p>
</div>`;
}
