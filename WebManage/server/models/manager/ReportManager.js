...
class ReportManager extends CommonModel{
    ...
    reportListSql(sqlCommandTable){
        return new Promise( ( resolve, reject ) => {
            ...
            if(sqlCommandTable.length==1){
                knex.raw(sqlCommandTable[0].sql).then(function(result) {
                    // console.log(result);
                    var dataReturn={};
                ...
                knex.raw(sqlCommandTable[0].sql).then(function(result) {
                    ...
                    var dataReturn={};
                    dataReturn[sqlCommandTable[0].table]=result;
                    knex.raw(sqlCommandTable[1].sql).then(function(result1) {
                        // console.log(result);
                ...
                knex.raw(sqlCommandTable[0].sql).then(function(result) {
                    ...
                    knex.raw(sqlCommandTable[1].sql).then(function(result1) {
                        // console.log(result);
                        dataReturn[sqlCommandTable[1].table]=result1;
                        knex.raw(sqlCommandTable[2].sql).then(function(result2) {
                            // console.log(result);
                ...
                knex.raw(sqlCommandTable[0].sql).then(function(result) {
                    ...
                    knex.raw(sqlCommandTable[1].sql).then(function(result1) {
                        ...
                        knex.raw(sqlCommandTable[2].sql).then(function(result2) {
                            // console.log(result);
                            dataReturn[sqlCommandTable[2].table]=result2;
                            knex.raw(sqlCommandTable[4].sql).then(function(result3) {
                                // console.log(result);
    ...
    reportPage(typePages){
        var report = squel.select().from("pages_content")
                    .where("group_file ='"+typePages+"'").where("deleteflag = 0")
                    .limit(100) ;
                //    console.log('reportPage reportPage');
                //    console.log(report.toString());
        return new Promise( ( resolve, reject ) => {
            knex.raw(report.toString()).then(function(result) {
               // console.log(result);
                resolve( result );
            }).catch(function(err){ 
               // console.log(err);
    ...
    deletePageContent(idset){
        var authen = squel.update().table("pages_content");
        authen.where('id='+idset)
        .set('deleteflag',1) 
    ...
    reportProductDetail(productId){
        var report = 'SELECT product.* , db.* ,dc.* FROM product LEFT JOIN product_sale_support db ON'
            +' db.productid=product.productid LEFT JOIN product_sale_km dc ON dc.id_km=db.idkm '
            +' WHERE product.productid='+productId;
        return new Promise( ( resolve, reject ) => {
            knex.raw(report).then(function(result) {
                resolve( result );
            }).catch(function(err){ return reject(err);} )
        } );
    }
    ...
    addProductPageDetail(content,link,userid){
      //  let content=link;
        ...
        var authen = squel.insert().into("pages_content")
        .set("userid",userid)
        .set("cost",cost)
    ...
    reportUserData(id){
        var authen = squel.select().from("users").where('userid='+id).where("deleteflag = 0");
        return new Promise( ( resolve, reject ) => {
            knex.raw(authen.toString()).then(function(result) {
                resolve( result );
            }).catch(function(err){ return reject(err);} )
    ...
    reportListSql(content,userid){
        ...
        {
            var authen = squel.select().from(tableSQL[i].table).where("deleteflag = 0");
    ...
    reportUserPage(page){
        var authen = squel.select().from("pages_content").where('id='+id).where("deleteflag = 0");
        return new Promise( ( resolve, reject ) => {
            knex.raw(authen.toString()).then(function(result) {
    ...
    reportDeivceToControl(tocken){
        var authen = squel.select().from("device_control").where('tocken='+tocken).where("deleteflag = 0");
        return new Promise( ( resolve, reject ) => {
            knex.raw(authen.toString()).then(function(result) {
                resolve( result );
            }).catch(function(err){ return reject(err);} )
    ...
    saveDetailProducttml(content,linkfile,idcreate){
        ...
        var authen = squel.insert().into("product_content")
    ...
    getThreshholdIndex(station) {
        var query = squel.select().from("sparc_sensor_max_min").where("deleteflag = 0");
        return new Promise( ( resolve, reject ) => {
    ...
    saveRecordWarning(station, content, userId) {
        var query = squel.insert().into("sparc_sensor_warning")
    ...
    getStatistics(fromTime, toTime, station_id) {
        var query = squel.select().from("sparc_sensor_data").field('station_id')
        .field("AVG(vbat)", "vbat_avr").field("MAX(vbat)", "vbat_max").field("MIN(vbat)", "vbat_min").field("SUM(vbat)", "vbat_sum")
        .field("AVG(broad_temp)", "broad_temp_avr").field("MAX(broad_temp)", "broad_temp_max").field("MIN(broad_temp)", "broad_temp_min").field("SUM(broad_temp)", "broad_temp_sum")
        .field("AVG(ch1_freq)", "ch1_freq_avr").field("MAX(ch1_freq)", "ch1_freq_max").field("MIN(ch1_freq)", "ch1_freq_min").field("SUM(ch1_freq)", "ch1_freq_sum")
        .field("AVG(ch1_temp)", "ch1_temp_avr").field("MAX(ch1_temp)", "ch1_temp_max").field("MIN(ch1_temp)", "ch1_temp_min").field("SUM(ch1_temp)", "ch1_temp_sum")
        .field("AVG(ch2_freq)", "ch2_freq_avr").field("MAX(ch2_freq)", "ch2_freq_max").field("MIN(ch2_freq)", "ch2_freq_min").field("SUM(ch2_freq)", "ch2_freq_sum")
        .field("AVG(ch2_temp)", "ch2_temp_avr").field("MAX(ch2_temp)", "ch2_temp_max").field("MIN(ch2_temp)", "ch2_temp_min").field("SUM(ch2_temp)", "ch2_temp_sum")
        .field("AVG(flag)", "flag_avr").field("MAX(flag)", "flag_max").field("MIN(flag)", "flag_min").field("SUM(flag)", "flag_sum")
        ...
        return new Promise( ( resolve, reject ) => {
            knex.raw(query.toString()).then(function(result) { 
                ...
                for(var i=0; i<result.length; i++) {
                    result[i].content = Buffer.from(result[i].content, 'base64').toString('ascii')
                }
                resolve( result);
            }).catch(function(err){ return reject(err);} )
        });
    ...
    getStation(stationId) {
        var query = squel.select().from("sparc_location_sensor").where("station_id = " + stationId);
        return new Promise( (resolve, reject) => {
