...
customerCtrl.getTableData =function (req, res) {
  ...
  if(!!tableSelect){
   ...
   var dataTableSQL=tableSelect.getSQLReport(req.currentUser) +" WHERE "+ tableSelect.getNameTable() +".deleteflag=0 "
                    + itemSelect
...
customerCtrl.addDataToTable= async  function (req, res) {
  ...
  if(!!tableSelect){
   ...
   authen.set("id_created",userid).set("id_updated",userid)
...
customerCtrl.updateData= async  function (req, res) {
  ...
  knex.raw(authen.toString()).then(function(x) {
      var authen2 = squel.update().table(tableSelect.getNameTable());
                        authen2.where(dataUser.locationSelect+'='+x[0].insertId)
      ...
      knex.raw(authen2.toString()).then(function(x) {
        ...
        for(var i=0;i<dataUser1.valueSetup.length;i++){
            ...
            authen1.set(item,data[item]);
        }
        //authen1.set("id_created",data[dataUser.userUpdate])
                // .set(dataUser.userUpdate,data[dataUser.userUpdate])
        authen1.set("id_updated",userid)
                    .set("created_at","NOW()",{dontQuote: true})
                    .set("updated_at","NOW()",{dontQuote: true})
                    .set("deleteflag",0)
                    .where(dataUser.locationSelect+'='+data[dataUser.locationSelect]);
        knex.raw(authen1.toString()).then(function(x) {
          return returnOK(res,x);
        }).catch(function(err){
...
customerCtrl.getAllInfoProduct  = async function(req, res) {
  var sql= "SELECT storeproduct.*,product.name,product.detail,product.image,product_image.* FROM storeproduct LEFT JOIN product on storeproduct.product_id=product.product_id LEFT JOIN product_image on product_image.image_id=storeproduct.product_image WHERE storeproduct.deleteflag =0 ";
  var x= await knex.raw(sql);
  if ((x!=null)&&(x.length>0)) {
      return returnOK(res,x[0]);
  }
...
customerCtrl.getInfoProduct  = async function(req, res) {
  var sql= "SELECT storeproduct.*,product.name,product.detail,product.image FROM storeproduct LEFT JOIN product on storeproduct.product_id=product.product_id WHERE storeproduct.deleteflag =0  AND product.store="+ req.body["type"];
  var x= await knex.raw(sql);
  if ((x!=null)&&(x.length>0)) {
      return returnOK(res,x[0]);
  }
...
customerCtrl.getDetailProduct  = async function(req, res) {
  console.log("req ...xxx....getDetailProduct...",req._parsedOriginalUrl.query );
  var sql= "SELECT product_image.*,product.name,product.detail,product.image FROM product_image LEFT JOIN product on product_image.product_id=product.product_id WHERE product_image.deleteflag =0 AND product_image.product_id="+ req._parsedOriginalUrl.query.replace('type=', '');
  var x= await knex.raw(sql);
  if ((x!=null)&&(x.length>0)) {
      return returnOK(res,x[0]);
  }
  ...
  try{
      ...
      if(!dataTocken.newUser){
        ...
        for(var i=0;i<dataUser.valueSetup.length;i++){
            ...
            addCustomer.set(item,data[item]);
        }
        addCustomer.set("id_created",0).set("id_updated",0)
        .set("created_at","NOW()",{dontQuote: true}) 
        .set("updated_at","NOW()",{dontQuote: true})
        .set("deleteflag",0);
      ...
      var sqlStringProduct= "INSERT INTO buyproductdetail (buyproduct_id,product_id,product_image, quantity, KM, created_at, updated_at, id_created, id_updated, deleteflag, oldid) VALUES ";
      var thefist=false;
      data.value.forEach(element => { 
          if(thefist) sqlStringProduct =sqlStringProduct +",";
          var addCustomer = '('+x[0].insertId+','+element.product_id+','+element.imageInfo.image_id+','+element.number+
...
customerCtrl.getDetailTheBill  = async function(req, res) {
  ...
  var sql= "SELECT buyproductdetail.*,product.name,product.detail,product_image.* FROM buyproductdetail LEFT JOIN product on buyproductdetail.product_id=product.product_id LEFT JOIN product_image on buyproductdetail.product_image=product_image.image_id WHERE buyproductdetail.deleteflag =0 AND buyproductdetail.buyproduct_id="+ req.body['bill'];
