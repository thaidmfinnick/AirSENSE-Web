...
class FoxDioLogger extends Interceptor {
  ...
  Future onRequest(RequestOptions options) async {
    ...
    if (requestHeader) {
      ...
      if (SettingInfo().GetUserInfor().tockenAcess != null) {
        options?.headers['Authorization'] = 'Bearer ${SettingInfo().GetUserInfor().tockenAcess}';
      }
    ...
    if (requestBody && options.method != 'GET') {
      ...
      if (data != null) {
        ...
        _map.addAll({'uri': uri});
        _map.addAll({'method': method});
        _map.addAll({'data': data});

        if (data is Map) _printMapAsTable(_map, header: 'Body');
        if (data is FormData) {
      ...
      _printMapAsTable(_map, header: 'Param');
    }
