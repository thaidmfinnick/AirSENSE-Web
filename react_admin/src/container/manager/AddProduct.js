...
import {addOneDataToTable,updateOneDataInfoTable} from '../../api/httpBaseUtil.js'
  ...
  const AddProduct = ({oldProduct=null}) => {
    ...
    const sendProduct=()=>{
      ...
      setLoading(true);
      currenInfo.storeproduct.company_id= currenInfo.product.company_id;
      ...
      sendDataInfo("product",currenInfo.product,currenInfo.product.product_id).then((product_id) => {
        ...
        currenInfo.storeproduct.product_id= product_id;
        currenInfo.product_image.forEach(element => {
    ...
    function sendStoreProduct(index){
      var currenInfo=state;
      setLoading(true);
      let data=currenInfo.storeproduct[index];
      sendDataInfo("storeproduct",data,data.id).then((infoproduct) => {
        currenInfo.storeproduct[index].store_product_id=infoproduct;
        currenInfo.storeproduct[index].id=infoproduct;
        /*currenInfo.storeproduct.push({store_product_id:0, product_id:currenInfo.product.product_id,
      ...
      <div className={'root-dm-message'}>
        ...
        {state.product.product_id? 
              <div>
                  ...
                  <StoreMaterial  info={state.storeproduct}
                        product_image={state.product_image}
                        idProduct={state.product.product_id}
                        companyId={state.product.company_id} 
                        changeData={(value)=>{onChange("product",value);}}
                        sendTemplate={(index)=>sendStoreProduct(index)}  />
              </div>
              :
              ""
        }
